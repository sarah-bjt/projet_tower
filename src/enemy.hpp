...
struct Enemy{
    ...
    void enemy_animation(float bullet_x, float bullet_y, int power_impact);   //méthode pour faire les sprites animés des ennemis (ils marchent)
    void enemy_death(float bullet_x, float bullet_y, int power_impact);   //méthode pour compter lorsqu'un ennemi est mort dû au joueur, et faire augmenter l'argent du joueur en fonction de money_reward
